...
- **Coger la plaza no cambia el rol** (14-sep-2026): `unirse_con_codigo` solo reactiva (`do update set active = true`); antes ponía `member` y un admin que cogía su nombre dejaba de serlo. **El traspaso no es editar**: `traspasar_plaza` pone `hitoo.traspasando_plaza` y `marcar_quien_edita` no toca `updated_by`; si no, todas las horas salían «editadas por» quien entraba. Probado de punta a punta en DEMO.
- **Equipo** enseña de cada plaza sin coger sus horas, la última y el correo, con `horas_por_persona(espacio)` (una llamada, `is_member`).
...
- **Importar de Clockify**: CSV del informe detallado, uno o varios a la vez -la versión gratuita solo exporta un año cada vez-, cada uno con su propio formato de fecha. Reimportar no duplica (`source = 'clockify'` + `external_id` = `persona|inicio|fin`, en la zona del espacio); si la misma persona tiene dos filas con el mismo inicio y el mismo fin, la segunda lleva el número de aparición detrás (`|2`, `|3`...). La columna «Client» entra como área, porque no hay clientes. Quien no tiene cuenta entra por defecto como plaza con sus horas. Desde el 14-sep-2026: lo ya importado se lee en cuanto hay ficheros, así que el resumen cuenta solo lo nuevo y no crea plazas ni catálogo para horas que ya estaban; cada persona se coloca sola por correo, por nombre (cuenta o plaza) o por dónde fueron sus horas en otra importación (una segunda cuenta de Clockify juntada a mano con la primera); el catálogo se carga con lo archivado, que si no se duplicaba; y los proyectos nuevos cogen colores de `COLORES_PROYECTO`, porque Clockify no exporta el color.
