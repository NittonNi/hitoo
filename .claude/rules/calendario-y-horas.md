...
- **Horas de otra persona** (14-sep-2026): `/panel?persona=` y `/calendario?persona=` para quien `veTodo` (admin, responsable, coach), con `selector-persona-vista.tsx` («Horas de»). Solo mirar: sin barra ni propuestas, sin play ni menú (`soloLectura` en `ListaEntradas`/`FilaEntrada`, `editable={false}` en `ColumnaDia`), y el cambio de semana conserva `persona`. Salen también las plazas sin coger. Si el id no es del espacio, las tuyas. El coach, en `/calendario`, siempre solo mira; en `/panel` sin `persona` sigue yendo a Estadísticas. Corregir horas ajenas, en Informes.
- **Semanas anteriores en el cronómetro**: `/panel` carga desde un lunes (el 1 del mes o hace 20 días) y `?desde=` baja ese lunes. «Ver semanas anteriores» salta a las cuatro semanas que acaban en la última hora de antes (`ultimaFechaAntes`): con una plaza de Clockify de hace un año, ir de cuatro en cuatro eran veinte clics por semanas vacías. Todo por URL, así `router.refresh()` tras corregir sigue enseñando lo mismo.
...
- **Duplicar una hora** (`fila-entrada.tsx`) cae justo despues: la copia empieza donde acaba la original (`entrada.end_at`) y dura lo mismo, contado con componentes de reloj en la zona del espacio (`addSecondsInZone` de `time.ts`, con `espacio.timezone`) y no sumando milisegundos -un dia de cambio de hora no son 24 h, y el rato puede cruzar la medianoche. Las cuentas de zona viven en `time.ts`: no se copian en los componentes. Se copian tambien la edicion, la tarea y las etiquetas (la insercion de etiquetas lleva `.select()` y avisa aparte si falla, sin deshacer la copia ya hecha).
- **Personas fijas en `selector-personas.tsx`**: la prop `fijas` (`PersonaFija[]`, con `id`, `nombre` y un `motivo` corto) pinta gente que ya cuenta y no se puede quitar desde ahi -p.ej. quien te propuso el rato (`venida_de_id`)-, marcada arriba del resto y contando en el texto del boton (si no, "Solo para mi" mentiria). Quien va en `fijas` no tiene que ir tambien en `miembros`, o saldria dos veces.
- **Reloj que corre y se pinta en texto** (segundos que cambian cada tick): servidor y navegador calculan el segundo en instantes distintos, asi que el primer pintado no coincide y React avisa de un desajuste de hidratacion. Se arregla con `suppressHydrationWarning` en el nodo de texto que cambia, nunca vaciando el numero. Lo llevan `EditorInicio` (`barra-cronometro.tsx`) y los tres relojes de `armazon.tsx` (menú plegado, barra lateral y pastilla de la cabecera móvil).
